...
void Parameters::load(ros::NodeHandle &nh) {
  nh.getParam("firmware/motors/encoder_resolution", &wheel_encoder_resolution, 1,
              TIMEOUT);
  nh.getParam("firmware/motors/torque_constant", &wheel_torque_constant, 1,
              TIMEOUT);
  nh.getParam("firmware/motors/pid/p", &wheel_pid_p, 1, TIMEOUT);
  nh.getParam("firmware/motors/pid/i", &wheel_pid_i, 1, TIMEOUT);
  nh.getParam("firmware/motors/pid/d", &wheel_pid_d, 1, TIMEOUT);
  nh.getParam("firmware/motors/pwm_duty_limit", &wheel_pwm_duty_limit, 1, TIMEOUT);
